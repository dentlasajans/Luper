import { WarningCircle, Warning, Package, Check, SpinnerGap, Monitor, MagnifyingGlass, Trash, X } from '@/src/components/ui/Icons';
...
const DebloatAppCard = memo(({ app, processingState, handleUninstall }: DebloatAppCardProps) => {
  ...
  return (
    <motion.div 
      ...
      className="bg-[#161618]/60 backdrop-blur-xl border border-white/[0.08] rounded-2xl p-4 flex items-center group hover:bg-white/[0.05] hover:border-white/[0.1] transition-all"
    >
      ...
      <div className="flex-1 min-w-0 ml-4">
        ...
        <div className="flex items-center mt-1.5 space-x-4 opacity-60">
          <p className="text-text-muted text-[12px] truncate max-w-[200px]" title={app.publisher}>
            {app.publisher || 'Bilinmeyen Yayinci'}
          </p>
          {app.version && (
      ...
      <div className="shrink-0 ml-4">
        <button
          ...
          className="p-2.5 rounded-xl bg-white/[0.04] hover:bg-[#ff5f56]/10 hover:text-[#ff5f56] text-text-muted transition-all duration-300 border border-white/[0.04] hover:border-[#ff5f56]/20 focus-visible:ring-2 focus-visible:ring-luper-primary focus-visible:outline-none"
          title="Uygulamayi Kaldir"
        >
          {processingState[appId] === 'processing' ? (
            ...
            <Check weight="duotone" size={16} className="text-luper-success" />
          ) : (
...
export function DebloatTools() {
  ...
  const handleUninstall = useCallback(async (app: InstalledApp) => {
    ...
    if (!window.confirm(`${app.name} programini bilgisayarinizdan kaldirmak istediginize emin misiniz?`)) {
      return;
    }
    ...
    try {
      ...
      setToastMessage({ type: 'success', message: `${app.name} basariyla kaldirildi veya kaldirma araci baslatildi.` });
      ...
      setToastMessage({ type: 'error', message: (err as Error).message || 'Kaldirma islemi basarisiz.' });
      setTimeout(() => setToastMessage(null), 4000);
    ...
    <div className="p-8 w-full h-full flex flex-col relative" style={{ WebkitAppRegion: 'no-drag' } as any}>
      ...
      >
        <div className="flex flex-col mb-8 pt-1 shrink-0">
          <h1 className="text-[32px] font-bold text-[#f5f5f7] tracking-tight leading-snug mb-2">Debloat & Uygulamalar</h1>
          <p className="text-text-muted/90 text-[14px] mt-2 leading-relaxed max-w-3xl">
            Bilgisayarinizda yüklü olan masaüstü ve magaza (UWP) uygulamalarini görüntüleyin ve gereksizleri kaldirarak sisteminizi rahatlatin.
          </p>
        </div>
          ...
          {loading ? (
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center bg-white/[0.04] border border-[#e57373]/30 rounded-3xl p-10 max-w-md">
                ...
                <h3 className="text-[#f5f5f7] text-[16px] font-medium leading-tight mb-3">Uygulamalar Alinamadi</h3>
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center bg-white/[0.04] border border-white/[0.08] rounded-3xl p-10 max-w-md">
                ...
                <h3 className="text-[#f5f5f7] text-[16px] font-medium leading-tight mb-3">Uygulama Bulunamadi</h3>
                <p className="text-text-muted text-[14px] font-normal leading-relaxed max-w-sm mx-auto">
                  Arama kriterlerinize uyan bir uygulama bulunamadi veya sisteminizde yüklü program yok.
                </p>
              </div>
            </div>
          )}
