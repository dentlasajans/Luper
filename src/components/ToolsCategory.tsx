import { Package, Power, Recycle } from '@phosphor-icons/react';
...
const ToolCard = React.memo(function ToolCard({ id, icon: Icon, title, description, improvements, onClick }: ToolCardProps) {
  ...
  return (
    <div 
      onClick={() => onClick(id)}
      className={`bg-[#18181c] border border-white/[0.06] shadow-[inset_0_1px_0_0_rgba(255,255,255,0.04)] ${lowQualityMode ? '' : 'backdrop-blur-xl'} rounded-2xl p-6 hover:bg-[#202024] hover:border-white/[0.12] hover:shadow-[inset_0_1px_0_0_rgba(255,255,255,0.08),0_8px_24px_rgba(0,0,0,0.4)] transition-all duration-300 group cursor-pointer flex flex-col h-full focus-visible:ring-2 focus-visible:ring-[#1a5efd] focus-visible:outline-none relative overflow-hidden`}
    >
...
export const ToolsCategory = React.memo(function ToolsCategory({ setActiveTab }: { setActiveTab: (tab: string) => void }) {
  const cards = useMemo(() => [
    {
      id: 'startup',
      title: 'Başlangıç Yöneticisi',
      description: 'Sistem açılışında otomatik başlayan gereksiz uygulama ve servisleri devre dışı bırakın.',
      icon: Power,
      improvements: ['Daha hızlı önyükleme', 'Kaynak tasarrufu']
    },
    {
      id: 'debloat',
      title: 'Kaldırıcı - Debloat',
      description: 'Windows ile birlikte gelen ancak kullanılmayan bloatware (gereksiz yazılımları) kaldırın.',
      icon: Package,
      improvements: ['Disk alanı açma', 'Arka plan işlem azalması']
    },
    {
      id: 'cleaner',
      title: 'Temizlik',
      description: 'Geçici dosyalar, önbellek ve gereksiz kayıt defteri girdilerini temizleyerek sisteminizi hafifletin.',
      icon: Recycle,
      improvements: ['Sistem ferahlığı', 'Performans artışı']
    }
  ], []);
    ...
    <div className="p-6 w-full h-full overflow-y-auto" style={{ maskImage: 'linear-gradient(to bottom, transparent 0%, black 3%, black 97%, transparent 100%)', WebkitMaskImage: 'linear-gradient(to bottom, transparent 0%, black 3%, black 97%, transparent 100%)', WebkitAppRegion: 'no-drag' }}>
      ...
      >
        <div className="mb-8 px-1 pt-1">
          ...
          <p className="text-text-muted/90 text-[14px] mt-2 leading-relaxed max-w-3xl">Sistem yönetimi, temizlik ve bakım araçları.</p>
        </div>
