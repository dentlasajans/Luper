...
class CrashPlatformEngineCore {
  ...
  getCrashHistory() {
    try {
      ...
      logError('[CrashPlatformEngine] Failed to read crash database:', { error: (e as Error).message });
    }
    return [];
  }

  saveCrashHistory(crashes: unknown) {
    try {
      const p = this.getStoragePath();
      fs.writeFileSync(p, JSON.stringify(crashes, null, 2), 'utf8');
      return true;
    } catch (e) {
      logError('[CrashPlatformEngine] Failed to save crash database:', { error: (e as Error).message });
      return false;
    }
  }
   ...
   */
  recordCrash(type: unknown, error: unknown, component: unknown = 'main_process') {
    ...
    // @ts-expect-error - auto fixed
    const recentCrashes = history.filter((c: unknown) => Date.now() - new Date(c.timestamp).getTime() < 5 * 60 * 1000);
