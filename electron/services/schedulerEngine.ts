...
class OptimizationSchedulerEngineCore {
  ...
  getTaskHistory() {
    try {
      ...
      logError('[SchedulerEngine] Failed to read task history', { error: (e as Error).message });
    }
    return [];
  }

  saveTaskHistory(history: unknown) {
    try {
      const p = this.getStoragePath();
      fs.writeFileSync(p, JSON.stringify(history, null, 2), 'utf8');
      return true;
    } catch (e) {
      logError('[SchedulerEngine] Failed to save task history', { error: (e as Error).message });
      return false;
    }
  }
   ...
   */
  cancelTask(taskId: unknown) {
    // @ts-expect-error - auto fixed
    const index = this.queue.findIndex((t: unknown) => t.id === taskId);
    if (index !== -1) {
      // @ts-expect-error - auto fixed
  ...
  async processQueue() {
    ...
    try {
      ...
      // @ts-expect-error - auto fixed
      const targetIds = plan.orderedPackages.map((p: unknown) => p.meta.id);
      ...
      logError(`[SchedulerEngine] Failed task [${task.id}]:`, { error: (error as Error).message });
