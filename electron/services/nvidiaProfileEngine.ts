...
export async function applyNvidiaProfileMode(mode: unknown) {
    ...
    if (!settingsConfig[mode]) {
        throw new Error('Geçersiz mod. Beklenen: fps | aaa-quality | aaa-fps | balanced');
    }

    // @ts-expect-error - auto fixed
    const targetSettings = settingsConfig[mode];
    
    let settingsXML = targetSettings.map((s: { id: string, val: string | number }) => `
      <ProfileSetting>
        <SettingNameInfo />
        // @ts-expect-error - auto fixed
  ...
  <Profile>
    ...
    try {
        ...
        if (!fs.existsSync(exePath)) {
            throw new Error('NVIDIA Profile Inspector bulunamadı. Lütfen uygulamayı yeniden kurun.');
        }

        await execFileAsync(exePath, [tmpFilePath, '-silent']);
        return true;
    } catch (e) {
        logError('NVIDIA Profile Error:', { error: (e as Error).message });
        return false;
    } finally {
        // Temp dosya sızıntısını önle
