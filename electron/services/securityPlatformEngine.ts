...
class SecurityPlatformEngineCore {
   ...
   */
  sanitizeRegPath(str: unknown) {
    if (!str || typeof str !== 'string') return '';
    const trimmed = str.trim();
    if (!/^(HKLM|HKCU|HKEY_LOCAL_MACHINE|HKEY_CURRENT_USER)(?:\\[a-zA-Z0-9_\-\s]+)*$/.test(trimmed)) {
      return '';
    }
    return trimmed;
  }
   ...
   */
  encryptSecret(plainText: unknown) {
    try {
      ...
      logError('[SecurityPlatformEngine] Encryption error:', { error: (e as Error).message });
   ...
   */
  decryptSecret(encryptedObj: unknown) {
    try {
      // @ts-expect-error - auto fixed
      ...
      logError('[SecurityPlatformEngine] Decryption error:', { error: (e as Error).message });
   ...
   */
  verifyPluginTrust(manifest: unknown) {
    // @ts-expect-error - auto fixed
    ...
    // @ts-expect-error - auto fixed
    const hasForbidden = (manifest.permissions || []).some((p: unknown) => dangerousPermissions.includes(p));
  ...
  getSecurityHealthScore() {
    ...
    masterKey!: unknown;
    securityViolationsCount!: unknown;
}
