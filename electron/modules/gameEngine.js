...
class GameOptimizationEngineCore {
  ...
  async fetchInstalledGames() {
    ...
    try {
      ...
      logError('[GameOptimizationEngine] Error fetching games:', { error: e.message });
    }
   ...
   */
  setGameProfile(gameId, profileUpdate) {
   ...
   */
  async launchGameWithOptimization(gameId, launcher) {
    ...
    if (process.platform === 'win32') {
      try {
        ...
        logError(`[GameOptimizationEngine] Failed to launch game [${gameId}]:`, { error: e.message });
      }
    }
