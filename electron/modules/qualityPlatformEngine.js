...
class QualityPlatformEngineCore {
  ...
  initCoreValidators() {
    ...
    this.registerValidator('gate.reliability', async () => {
      ...
      const recentCount = crashes.filter(c => Date.now() - new Date(c.timestamp).getTime() < 10 * 60 * 1000).length;
  ...
  registerValidator(gateId, validatorFn) {
  ...
  async runContinuousQualityAudit() {
    ...
    for (const [gateId, validatorFn] of this.validators.entries()) {
      try {
        ...
        logError(`[QualityPlatformEngine] Quality Gate [${gateId}] exception:`, { error: err.message });
        gates.push({
          gateId,
          name: gateId,
          status: 'failed',
