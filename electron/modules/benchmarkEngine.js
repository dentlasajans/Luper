...
import { cachedStorageDrives, getCpuUsageInstant } from './systemInfo.js';
...
class BenchmarkEngineCore {
  ...
  getHistory() {
    try {
      ...
      logError('[BenchmarkEngine] Failed to read history', { error: e.message });
    }
    return [];
  }
  ...
  saveHistory(records) {
    try {
      ...
      logError('[BenchmarkEngine] Failed to write history', { error: e.message });
      return false;
    }
  }
   ...
   */
  startSession(profileName = 'Standard Optimization Benchmark') {
