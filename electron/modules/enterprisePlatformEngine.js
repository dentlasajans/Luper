...
class EnterprisePlatformEngineCore {
  ...
  getEnterpriseConfig() {
    try {
      ...
      logError('[EnterprisePlatformEngine] Failed to read enterprise config:', { error: e.message });
    }
    return {
      orgId: 'standalone',
  ...
  saveEnterpriseConfig(config) {
    try {
      ...
      logError('[EnterprisePlatformEngine] Failed to save enterprise config:', { error: e.message });
      return false;
    }
  }
   ...
   */
  isOptimizationAllowedByPolicy(packageMeta) {
